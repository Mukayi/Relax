...
- Each timer call (start / stop plus two `event.record()`) costs about 24 µs; replaying Megatron's call sequence with the worst case of 17 timers per step (3 micro-batches), a single-GPU benchmark measures 0.40 ms/step (max 0.46 ms).
- Adding these up and assuming all of it sits on the critical path gives about 0.93 ms/step typical and 1.54 ms/step worst case, i.e. 0.08 % and 0.13 % of a 1.15 s step. In a kernel-launch-bound loop the profiler adds 0.59 ms/step of wall time (including the end-of-step readout), consistent with the timer-call plus readout cost. This is a summed upper-bound estimate, not an end-to-end measurement.
